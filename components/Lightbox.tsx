...
export default function Lightbox({ items, startIndex = 0, title, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Title + counter */}
      <div className="absolute top-5 left-5 md:top-8 md:left-8 font-display text-[12px] tracking-[0.02em] text-white/60 z-10">
        {title && <span className="mr-3 text-white/90">{title}</span>}
        <span>{String(idx + 1).padStart(2, '0')} / {String(count).padStart(2, '0')}</span>
      </div>
      ...
      {count > 1 && (
        <button
          ...
          className="absolute left-3 md:left-6 top-1/2 -translate-y-1/2 w-12 h-12 grid place-items-center text-white/70 hover:text-white border border-white/20 hover:border-white/60 rounded-full transition-colors z-10"
          aria-label="Previous"
        >
          <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor"><path d="M9 1 L3 7 L9 13 Z" /></svg>
        </button>
      )}
      ...
      {count > 1 && (
        <button
          ...
          className="absolute right-3 md:right-6 top-1/2 -translate-y-1/2 w-12 h-12 grid place-items-center text-white/70 hover:text-white border border-white/20 hover:border-white/60 rounded-full transition-colors z-10"
          aria-label="Next"
        >
          <svg width="14" height="14" viewBox="0 0 14 14" fill="currentColor"><path d="M5 1 L11 7 L5 13 Z" /></svg>
        </button>
      )}
